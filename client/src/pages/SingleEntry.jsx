...
import AlertDialog from '../components/ConfirmationBox';
...
const SingleEntry = () => {
    ...
    const getLatestData = async () => {
        try {
            ...
            setdropdown(res.data.extra.dropDown);
        } catch (error) {
            console.log(error);
        }
    }
    ...
    const deleteEntry = async () => {
        try {
            const res = await axios.post(`${uri}/delete-entry`, {id: formData._id});
            if (res.data.success) {
    ...
    return (
        <Container>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                { currentUser.isAdmin &&
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'darkred' }}>
                    <AlertDialog id={formData._id} deleteEntry={deleteEntry}/>
            ...
            <Paper style={{ padding: '16px', marginBottom: '16px' }}>
                {nonEditableFields.map(field => {
                    if (field == "dateOfDemand") {
                        let date = new Date(formData[field]);
                        let formattedDate = new Intl.DateTimeFormat('en-GB').format(date);
                        return (
                            <Typography variant="body1" key={field}>
                                <strong>{displayName(field)}:</strong> {formattedDate}
                            </Typography>
                        )
                    }
                    return (
